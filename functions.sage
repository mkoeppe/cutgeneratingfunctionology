    ...
    def __init__(self, fdms=[], covered_components=[], proj_add_vert=set(), show_plots=False, plot_background=None, function_at_border=None, pts_of_discontinuity=None, show_zero_perturbation=True):
        ...
        Input for abstract move ensembles:
         ...
         - pts_of_discontinuity: Either ``True'' (everywhere discontinuous), or a list of points
           ...
           If ``function_at_border`` is provided, uses the allowed discontinuities of its effective perturbations.
           Otherwise, defaults to ``True`` (everywhere discontinuous).

        Additional input for move ensembles coming from a function:
         - function_at_border: a ``FastPiecewise`` function
        """
        ...
        if function_at_border is not None:
            if pts_of_discontinuity is not None:
                raise ValueError("only one of function_at_border and pts_of_discontinuity can be provided")
