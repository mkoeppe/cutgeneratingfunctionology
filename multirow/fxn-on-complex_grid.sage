...
def FastPiecewise_to_2d_diagonal_to_function_on_complex_grid(f):
    """
    ...
    #printPolyFxnPair(l)
    return function_on_complex_grid(l)


def FastPiecewise_to_2d_strip_to_function_on_complex_grid(f):
    """
...
def FastPiecewise_to_function_on_complex_grid(f,cont=True):
    """
...
class function_on_complex_grid :
    ...
    def __init__(self, l,cont=True):
        ...
        #construct grid for all polyhedrons
        self.polyhedralArrangement = PolyhedralArrangement(self.polyhedronFxnDict.keys())
    ...
    def _polyhedron_value_pairs_containing(self,point):
        ...
        #print point,plist
        #Use bucket search to find all polyhedrons that contain this point
        l = []  # A list of such polyhedron value pairs
        for pt in plist:
            intersect_poly = self.polyhedralArrangement.point_in_polyhedra(pt)
            for p in intersect_poly:
                evaluatedPair = (p,self.evaluatePoint(self.polyhedronFxnDict[p],pt))
    ...
    def _limit(self,point,T): 
        ...
        """
        #Check if point and any of its periodic is in T
        plist = addPeriodicPoints(point)
        inside = False
        for pt in plist:
            if inside: 
    ...
    def plot_2d_complex(self, discontWidth = 1/60):
        ...
        for pair in self.polyhedronFxnPair:
            ...
            if g not in gradientDic:
                gradientDic[g] = index
                index += 1

        #colors list
        colors = rainbow(len(gradientDic))
        ...
        xtick_formatter = [ "$%s$" % latex(x) for x in t1 ]
        ytick_formatter = [ "$%s$" % latex(y) for y in t2 ]
        ...
        for pair in self.polyhedronFxnPair:
            ...
            if pair[0].dim() == 2:
                ...
                g = tuple(pair[1].gradient())
                color = colors[gradientDic[g]]

                draw = self.calculateWhiteStrip(pair,discontWidth)
                shrinkedPolyhedron = draw[0]
                if g not in set(labeled):
                    image += plot(shrinkedPolyhedron,wireframe=False, fill=color, alpha=0.5,ticks=[list(t1),list(t2)],tick_formatter = [xtick_formatter, ytick_formatter],zorder = 1, legend_label = "Gradient %s" % str(g))
                    labeled.append(g)
                else:
                    image += plot(shrinkedPolyhedron,wireframe=False, fill=color, alpha=0.5,zorder = 1)
                for s in draw[1]:
                    image+= s
            #1d polyhedrons(lines)
                ...
                for polyhedra in neighborPoly:
                    ...
                    if self.value_matches_at_line(vert[0],vert[1],f,pair[1]):
                        #if midpoint_value == self.evaluatePoint(f,midpoint):
                        linecolor.append(colors[gradientDic[tuple(f.gradient())]])
                if len(linecolor) == 0:
                    linecolor.append('black')
                if len(linecolor) < 2:
                    image += line2d(pair[0].vertices_list(),rgbcolor=linecolor[0],thickness=2, zorder = 2)
                #print linecolor
                    ...
                    if polyVal[0].dim() == 2 and self.evaluatePoint(polyVal[1],point) != point_value:
                        ...
                        if list(point) not in polyVal[0].vertices_list():
                            ...
                            if not semicircle:
                                ...
                                image += circle(point,calculatedWidth,rgbcolor = 'white',fill = True, facecolor = 'white',zorder = 2)
                            else:
                                image += semicircle

                image += point2d(pair[0].vertices_list(),rgbcolor='black',size = 10,zorder=3)
    ...
    def calculateSectorInPolyhedra(self,orig,edge1,edge2,radius,polyhedra):
        ...
        #print "point", key, mid_angle
        #print "testpoint", testPoint
        #print "v1", vector1, "from", edge1, angle1
        #print "v2",vector2, "from", edge2, angle2
    ...
    def construct_delta_pi_on_delta_p(self):
        ...
        Example::
            ...
            sage: printPolyFxnPair(F2.polyhedronFxnPair)
            [[2/3, 0], [0, 2/3], [2/3, 1/3], [1/3, 2/3]] 3/4
            [[1/3, 2/3]] 3/2*u0 + 1
            [[0, 2/3]] 0
            [[0, 0], [2/3, 0], [0, 2/3]] 0
            [[0, 2/3], [1/3, 2/3], [0, 1]] 0
            [[2/3, 1/3], [1/3, 2/3]] 3/2*u0 + 3/2*v0
            [[1, 0]] 0
            [[0, 2/3], [1/3, 2/3]] -3/2*v0 + 7/4
            [[0, 1], [1/3, 2/3]] 3/2*u0 + 3/2*v0 - 3/4
            [[0, 2/3], [2/3, 0]] 3/2*u0 + 3/2*v0 - 1
            sage: F2.vertices
            [(2/3, 0), (0, 2/3), (2/3, 1/3), (1/3, 2/3), (0, 1), (0, 0), (1, 0)]
        ...
        return function_on_complex_grid(delta_p_function_pair)
    ...
    def findFxn(self,p):
        ...
        """  
        polyList = self.addPeriodicPolyhedron(p)
        #print polyList[0].vertices_list()
        poly = self.polyhedralArrangement.lowest_dim_contains(polyList[0])

        for polyhedra in polyList[1:]:
            tmp = self.polyhedralArrangement.lowest_dim_contains(polyhedra)
            if tmp.dim() < poly.dim():
                poly = tmp
        ...
        if poly == None:
            ...
            #print p.vertices_list(), "is in", poly.vertices_list()
            return self.polyhedronFxnDict[poly]
    ...
    def createFIJK(self, a, b, c) :
        ...
        #print 'A:',A.vertices(),'B:',B.vertices(),'C:',C.vertices()
        return FIJK(A,B,C)
...
def stripWidth_upper_bound_given_edge(v1,v2,polyhedra):
    """
    Given a line from v1 to v2, which is an edge of the polyhedra
...
class function_on_standard_2d_triangulation(function_on_complex_grid):
