...
def psi_gj_conjecture_counterexample(f=2/3, q=4, eps=1/1000, maxiter=10000):
    """
    ...
    Parameters:
        ...
        eps (real), 0 <= eps < 1 : to control the serie sum;
        maxiter (integer) : maximum number of iterations;

    Note:
        psi is the uniform limit of the sequence of functions psi_n, 
        generated by psi_n_in_gj_conjecture_counterexample_construction(f, [e[0], e[1], ..., e[n - 1]]).
        e is a geometric series with ratio q, such that: 
        0 < ... < e[n] <= e[n - 1] <= ... <= e[1] <= e[0] <= 1 - f and \sum_{i = 0}^{\infty} {2^i * e[i]} <= f.
        The first n terms of e are generated by generate_example_e_for_psi_n(f, n, q, eps)
        
    See also:
        def generate_example_e_for_psi_n(f, n q, eps)
        def psi_n_in_gj_conjecture_counterexample_construction(f, e)

    Examples:
        quick evaluations::
            sage: psi_gj_conjecture_counterexample(f=2/3, q=4, eps=0, maxiter=10000)(r=1/5)
            sage: psi_gj_conjecture_counterexample(f=2/3, q=4, eps=0, maxiter=10000)(r=1/4)

        too many iterations::
            sage: psi_gj_conjecture_counterexample(f=2/3, q=4, eps=0, maxiter=10000)(r=9/40)
    ...
    def evaluate_psi_at_r(r):
        ...
        if bool (f <= r <= 1):
            return (1 - r) / (1 - f)
        if bool (0 < r < f):
            z = (1 - eps)*(q - 2) / q * min(f, 1 - f)
            # or take z = min((1 - eps)*(q - 2)*f / q , 1 - f) 
            n = 0
            x_left = 0
            x_right = f
            y_left = 0
            ...
            while not (bool((x_left + x_right - z)/2 <= r <= (x_left + x_right + z)/2)) and (n < maxiter):
                ...
                n += 1
            if n == maxiter:
                logging.info("Reaching max number of iterations, return approximate psi(%s)" %r)   
