...
def discontinuous_bhk_irrational_dense_move_not_affine():
    ...
    vv = [v[0]+1/500, v[1], v[2]+1/500, v[3]-1/500, v[4]+1/500, v[5]-1/500, v[6]+1/500]+\
v[7:13]+[v[13]-1/500,v[14]+1/500,v[15]-1/500,v[16]+1/500,v[17]-1/500,v[18],v[19]-1/500,v[20]+1/500]
    discp = []
    for i in range(20):
...
def discontinuous_bhk_irrational_dense_move_is_affine():
    """
    Non extreme function that is two-sided discontinuous at the origin and has dense move. Similar to discontinuous_bhk_irrational_dense_move_not_affine(), but dense move implies affine linear in this example.
    The 2-d face F that is above and c.ontains the edge {3/10 <= x <= 7/20, y = 3/20} has \Delta\pi_F(3/10, 3/20)=\Delta\pi_F(7/20, 3/20)=0. By Lemma 8.4(2), \tilde\pi is continuous between 3/10 and 7/20, thus crazy fat perturbation \tilde\pi doesn't exist and \tilde\pi is affine on (7/20, 3/20) and (9/20, 1/2).
    """
...
def minimal_delta_pi_has_additive_limit_vertex():
    """
    \Delta\pi=0 at limit vertices (1/4+, 1/4+, 1/2+), (1/4+, 1-, 5/4-) and (1-, 1/4-, 5/4-). By Lemma 8.4(2), \tilde\pi must be uniformly continuous on (1/4-\delta, 1/4), (1/4, 1/4+\delta), (1/2, 1/2+\delta) and (1-\delta, 1) for some small \delta > 0.
    """
    h =  FastPiecewise([singleton_piece(0,0), open_piece((0,1/2), (1/4, 3/4)), singleton_piece(1/4, 1/2), open_piece((1/4, 1/4), (1/2, 1/2)), singleton_piece(1/2, 1), open_piece((1/2, 1/2), (1, 1/2)), singleton_piece(1,0)])
    #p = plot_2d_complex(h)+plot_function_at_borders(h)+plot_projections_at_borders(h)
    p = plot_2d_diagram(h)
    additive_vertices = generate_additive_vertices(h, reduced=False)
