...
class BasicSemialgebraicSet_groebner_basis(BasicSemialgebraicSet_base):
    ...
    EXAMPLES::
        ...
        sage: bsa_eq_lt_le = BasicSemialgebraicSet_eq_lt_le_sets.from_bsa(veronese)
        sage: bsa_eq_lt_le
        BasicSemialgebraicSet_eq_lt_le_sets(eq=[z + 2, y + 1], lt=[x^2 + y*z - 11], le=[])
        sage: bsa_gb = BasicSemialgebraicSet_groebner_basis(veronese)
        sage: bsa_gb
        BasicSemialgebraicSet_groebner_basis(eq=[y + 1, z + 2], lt=[x^2 - 9], le=[])
        sage: bsa_gb.add_polynomial_constraint(x-2, operator.eq)
        sage: bsa_gb
        ...
        [x - 2, z + 2, y + 1]
        sage: bsa_gb2 = BasicSemialgebraicSet_groebner_basis(bsa_eq_lt_le)
        sage: bsa_gb2.add_polynomial_constraint(x^3-8, operator.eq)
        sage: bsa_gb2
        BasicSemialgebraicSet_groebner_basis(eq=[x^3 - 8, y + 1, z + 2], lt=[x^2 - 9], le=[]
        sage: bsa_gb2.add_polynomial_constraint(x^5-32, operator.eq)
        sage: bsa_gb2
        BasicSemialgebraicSet_groebner_basis(eq=[x - 2, y + 1, z + 2], lt=[], le=[])
